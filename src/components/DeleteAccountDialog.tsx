...
const DeleteAccountDialog = ({ open, onOpenChange, onConfirm }: Props) => {
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Trigger asChild>
        <button className="text-red-400 py-2 px-3 rounded-xl mt-4 hover:underline">Delete Account</button>
      </Dialog.Trigger>
      <Dialog.Portal>
        ...
        >
          <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#1e1e1e] p-3 md:p-6 rounded-md max-w-sm w-[90%] space-y-4 md:mx-0 ">
            <Dialog.Title className=" text-lg md:text-2xl text-white">Delete account?</Dialog.Title>
